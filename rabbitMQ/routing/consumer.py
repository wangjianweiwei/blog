...
def main():
    ...
    # 创建交换机
    exchange = "logs_routing"
    channel.exchange_declare(exchange, exchange_type="direct")
    # 将队列绑定到交换机上, 并指定routing_key(一个队列可以绑定多个routing_key)
    for routing_key in sys.argv[1:]:
        channel.queue_bind(queue=queue.method.queue, exchange=exchange, routing_key=routing_key)
